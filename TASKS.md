...
### 模拟器设计差距（基于论文第5章分析）
- [ ] **事件状态更新模型**：当前 `ProcessManager::drive_state_transitions` 只做状态转换，未实现"占用时消费输入信号无效化、完成时输出有效化、结束时释放"的信号-状态联动。信号无效化依赖各模块 `check_*` 手工调用 `invalidate_signal`，应收敛为框架统一管理（对照论文 5.2.2 事件状态更新模型）
- [ ] **事件计数器自动化**：当前事件由模块条件函数动态触发，未实现从推理任务参数 + 架构参数静态生成事件集合（对照论文 5.1.2 / 算法 5.1）
- [ ] **组合逻辑依赖检测**：框架未自动检测延迟为 0 的组合逻辑功能函数并插入高优先级事件栈优先执行（对照论文 5.2.2 / 算法 5.3 第19行）
- [ ] **反馈时序依赖数据队列**：未实现数据入队机制防止"后级在 >T 时刻修改前级信号覆盖当前有效数据"，正确反压下队列长度 ≤2（对照论文 5.2.2）
- [x] **多核事务机制泛化**：新增 `TaskDependency.h`（任务依赖原语：`TaskDependencyEntry` 生产-消费屏障 + `TaskDependencyTable` 活跃任务表）；`CycleAccurateSimulator` 集成 `register_task_dependency`/`on_task_done`；`MulticoreSimulator` 6 核硬编码 switch 迁移为 4 个数据驱动表项（E1-E4），并修复 B6/B7 清零不完整 bug；支持不同生产者不同阈值、单生产者触发多消费者等通用场景；新增 taskdependency_test（8 用例）
- [ ] **通信系统建模接口**：模拟器内无 NoC 通信建模接口（专用模拟器集成或统计模型），当前 NoC 延迟由 Python 侧 Booksim 估算（对照论文 5.2.3 通信系统）
- [ ] **存储粒度一致性**：存储模块数据填充事件粒度应与计算粒度一致（特征图分块/列），避免过细粒度踪迹事件降低模拟性能（对照论文 5.2.3 存储系统）
