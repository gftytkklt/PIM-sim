...
- Message system: framework only defines `GenericMessage` structure (task_id/body/delay), does NOT preset message types. Users register typed handlers via `register_message_handler<T>(task_id, std::function<void(const T&)>)` (module-level) or `register_task_handler<T>(task_id, std::function<void(const T&)>)` (simulator-level); framework auto-registers `type_index(T)` and validates on send/dispatch. Untyped handlers (`void(const GenericMessage&)`) still supported for backward compat.
