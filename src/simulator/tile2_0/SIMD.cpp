...
SIMD::SIMD(const std::string& id) : ModuleBase(id) {
    // Crossbar接口
    add_signal(Signal(SignalID::SIMD_channel_batch, Signal::Direction::INPUT)); // int, input batch num
    add_signal(Signal(SignalID::SIMD_compute_ready, Signal::Direction::OUTPUT, std::make_any<bool>(true))); // bool, output compute ready signal
    // TS接口
    add_signal(Signal(SignalID::SIMD_pooling_enable, Signal::Direction::INPUT)); // bool, input pooling enable signal
    add_signal(Signal(SignalID::SIMD_data_valid, Signal::Direction::OUTPUT, std::make_any<bool>(false))); // bool, output data valid
    set_signal_value(SignalID::SIMD_compute_ready, true, 0); // 初始化时SIMD处于ready状态
}

bool SIMD::check_computation_trigger() {
    auto channel_batch_val = get_signal_value(SignalID::SIMD_channel_batch);
...
bool SIMD::check_computation_exec() {
    auto SIMD_ready = get_signal_value(SignalID::SIMD_compute_ready);
    if (SIMD_ready.has_value() && std::any_cast<bool>(SIMD_ready)) {
        ...
        submit_signal_value(SignalID::SIMD_compute_ready, false, 1); // 计算开始后重置ready信号
        auto channel_batch_val = get_signal_value(SignalID::SIMD_channel_batch);
        auto stage_num = (std::any_cast<int>(channel_batch_val) + channel_num_ - 1) / channel_num_; // 计算需要的阶段数
        auto pooling_enable_val = get_signal_value(SignalID::SIMD_pooling_enable);
        ...
        submit_signal_value(SignalID::SIMD_data_valid, true, compute_latency_); // 计算完成后数据有效
        return true;
    }
    return false;
}
...
bool SIMD::check_computation_finish() {
    auto data_valid = get_signal_value(SignalID::SIMD_data_valid);
...
bool SIMD::check_computation_end() {
    ...
    submit_signal_value(SignalID::SIMD_compute_ready, true, 1); // 计算完成后设置ready信号
    submit_signal_value(SignalID::SIMD_data_valid, false, 1); // 重置数据有效信号
    return true;
}
