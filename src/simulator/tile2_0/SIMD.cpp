...
SIMD::SIMD(const std::string& id) : ModuleBase(id) {
    ...
    add_signal(Signal("SIMD_compute_ready", Signal::Direction::OUTPUT, true)); // bool, output compute ready signal
    add_signal(Signal("SIMD_data_valid", Signal::Direction::OUTPUT, false)); // bool, output data valid
    // add_signal(Signal("SIMD_processing", Signal::Direction::INTERNAL, false));
}
