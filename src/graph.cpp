...
void CGraph::build_graph_subset(){
    ...
    for (size_t i = 0; i < kernels.size(); i++) {
        ...
        total_compute_num += get_compute_num(kernels[i].wsize, kernels[i].channel, kernels[i].ofmap_size);
    }
    ...
    std::cout << "[CG] CGraph subset built with " << kernels.size() << " kernels, max cnode num: " << max_cnode_num << ", total compute num: " << total_compute_num << ", max compute num: " << max_compute_num << std::endl;
    ...
    double compute_util = static_cast<double>(max_compute_num) / total_compute_num;
    std::cout << "Total cnode num of the kernels: " << total_cnode_num << ", cnode utilization: " << cnode_util << ", kernel utilization: " << kernel_util << ", compute utilization: " << compute_util << std::endl;
}
