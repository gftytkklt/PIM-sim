"""PIM-sim plotting and visualization module."""
...
import pickle
import os
from analysis import get_opt_str, perf_analysis, load_noc_perf, get_noc_perf
from MappingInfo import latency_est, booksim_eval
from onnx_analysis import make_hw_info, load_kernel
# 主调用函数
...
def plot_comm(
    ...
    base_values = (
        [
            comm_result[model][opt_combinations[0]].get(dict_key, 1e-6)
            for model in models
        ]
        if norm
        else None
    ...
    for i, opt in enumerate(opt_combinations):
        values = [comm_result[model].get(opt, {}).get(dict_key, 0) for model in models]
...
def plot_bw_perf(
    ...
    for bw in bw_list:
        ...
        for opt in opt_combinations:
            ...
            # values = [v/b for v, b in zip(values, base_values)]
            latency = (
                [v[0] / b[0] for v, b in zip(values, base_values)]
                if base_values
                else latency
            )
            throughput = (
                [v[1] / b[1] for v, b in zip(values, base_values)]
                if base_values
                else throughput
            )
            # print(latency)
...
def plot_brkdown(comm_segs, latency_dict, bw=1, threshold=0.1, ideal=0):
    ...
    for i, opt in enumerate(opt_combinations):
        ...
        cal_pers = [
            cal / (cal + lat) if (cal + lat) > 0 else 0
            for cal, lat in zip(cal_lat, lat_lat)
        ]
        lat_pers = [
            lat / (cal + lat) if (cal + lat) > 0 else 0
            for cal, lat in zip(cal_lat, lat_lat)
        ]
...
def plot_bw(comm_segs, latency_dict, bw=1):
    ...
    for idx, model in enumerate(models):
        ...
        util_per = [
            ideal_lat / comm_lat for ideal_lat, comm_lat in zip(ideal_lats, comm_lats)
        ]
    ...
    for ax, title, y_max in zip(
        [ax1, ax2],
        ["Normalized Bandwidth Utilization", "Ideal Comm Latency Improvement"],
        [max_util * 1.2, max_ideal * 1.2],
    ):
...
def plot_grouped_bars(
    ...
    # 统一图例（现在处理三组数据）
    handles = [
        plt.Rectangle((0, 0), 1, 1, fc=colors[i], ec="white") for i in range(n_groups)
    ]
    fig.legend(
        handles,
        group_labels,
...
def plot_xbarsize():
    mapping_result_512, _ = perf_analysis(
        models_dir="xbars", hwinfo=make_hw_info((512, 512), 2, (0, 0), 1)
    )
    mapping_result_256, _ = perf_analysis(
        models_dir="xbars", hwinfo=make_hw_info((256, 256), 8, (0, 0), 1)
    )
    mapping_result_128, _ = perf_analysis(
        models_dir="xbars", hwinfo=make_hw_info((128, 128), 32, (0, 0), 1)
    )
    ...
    # model = 'alexnet.onnx'
    opt_combinations = sorted(
        set(opt for opts in mapping_result_256.values() for opt in opts)
    )
    res_512 = [
        latency_est(
            mapping_res=mapping_result_512[model].get(opt, 0),
            bus_width=1,
            comm_lat=lat_dict_512[(model, opt)],
        )
        for opt in opt_combinations
    ]
    res_256 = [
        latency_est(
            mapping_res=mapping_result_256[model].get(opt, 0),
            bus_width=1,
            comm_lat=lat_dict_256[(model, opt)],
        )
        for opt in opt_combinations
    ]
    res_128 = [
        latency_est(
            mapping_res=mapping_result_128[model].get(opt, 0),
            bus_width=1,
            comm_lat=lat_dict_128[(model, opt)],
        )
        for opt in opt_combinations
    ]
    ...
    group_labels = ["128x128", "256x256", "512x512"]
    plot_grouped_bars(
        [lat_128, lat_256, lat_512],
        [throughput_128, throughput_256, throughput_512],
        group_labels=group_labels,
        bar_labels=bar_labels,
        save_path="results/xbarsize.pdf",
    )
...
def plot_pipeline():
    mapping_result_LS, _ = perf_analysis(
        models_dir="models", hwinfo=make_hw_info((256, 256), 8, (0, 0), 1)
    )
    mapping_result_LP, _ = perf_analysis(
        models_dir="models", hwinfo=make_hw_info((256, 256), 8, (0, 0), 10000)
    )
    ...
    base_LS = [
        latency_est(
            mapping_res=mapping_result_LS[model].get(opts[0], 0),
            bus_width=1,
            comm_lat=lat_dict_LS[(model, opts[0])],
        )
        for model in models
    ]
    base_LP = [
        latency_est(
            mapping_res=mapping_result_LP[model].get(opts[0], 0),
            bus_width=1,
            comm_lat=lat_dict_LP[(model, opts[0])],
            syn=0,
        )
        for model in models
    ]
    opt_LS = [
        latency_est(
            mapping_res=mapping_result_LS[model].get(opts[1], 0),
            bus_width=1,
            comm_lat=lat_dict_LS[(model, opts[1])],
        )
        for model in models
    ]
    opt_LP = [
        latency_est(
            mapping_res=mapping_result_LP[model].get(opts[1], 0),
            bus_width=1,
            comm_lat=lat_dict_LP[(model, opts[1])],
            syn=0,
        )
        for model in models
    ]
    ...
    throughput_base_LS = [
        throughput / base
        for throughput, base in zip(throughput_base_LS, throughput_base)
    ]
    throughput_base_LP = [
        throughput / base
        for throughput, base in zip(throughput_base_LP, throughput_base)
    ]
    throughput_opt_LS = [
        throughput / base
        for throughput, base in zip(throughput_opt_LS, throughput_base)
    ]
    throughput_opt_LP = [
        throughput / base
        for throughput, base in zip(throughput_opt_LP, throughput_base)
    ]
